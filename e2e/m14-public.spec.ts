...
test.describe("public tier — signed-out renders", () => {
  ...
  test("/changelog is the real M0→M16 history", async ({ page }) => {
    await page.goto("/changelog");
    await expect(page.getByRole("heading", { name: "What we shipped." })).toBeVisible();
    await expect(page.getByText("10 drops")).toBeVisible();
    await expect(
    ...
    await expect(page.locator("#m16").getByText("current")).toBeVisible();
